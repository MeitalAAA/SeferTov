           ...
           if (!is_wp_error($parsed_data)){
               Wt_Import_Export_For_Woo_Logwriter::write_log($this->parent_module->module_base, 'import', "Row :$row - Processing item.");
                ...
                if(!is_wp_error($result)){
                    ...
                    $this->import_results[$row] = array('row'=>$row, 'message'=>$msg, 'status'=>true, 'status_msg' => __('Success', 'wt-import-export-for-woo'), 'post_id'=>$result['id'], 'post_link' => Wt_Import_Export_For_Woo_Product_Review::get_item_link_by_id($result['id'])); 
...
//            update_option('xa_rev_im_ex_last_import_csv_end_col', $post_id);
    ...
    //            $this->add_import_result('imported', 'Import successful', $post_id);
            ...
            unset($post);
           do_action('wt_iew_review_import_inserted_data', $post, $post_id);
