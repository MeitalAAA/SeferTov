...
class Wt_Import_Export_For_Woo_Order_Bulk_Export {
    ...
    public static function do_export($post_type = 'shop_order', $order_IDS = array(), $xmldata = '0') {
        ...
        if (!empty($row)) {
            $row = array_map('Wt_Import_Export_For_Woo_Order_Bulk_Export::wrap_column', $row);
        ...
        foreach ($order_ids as $order_id) {
            ...
            if (!empty($data)) {
                $row = array_map('Wt_Import_Export_For_Woo_Order_Bulk_Export::wrap_column', $data);
                fwrite($fp, implode($delimiter, $row) . "\n");
            }
            unset($row);
            unset($data);
