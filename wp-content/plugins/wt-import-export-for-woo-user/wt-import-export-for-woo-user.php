...
add_filter('wt_iew_plugin_settings_tabhead', 'wt_user_mail_content_tab', 11, 1);

if (!function_exists('wt_user_mail_content_tab')) {

	function wt_user_mail_content_tab($arr) {
		$out = array();
		foreach ($arr as $key => $value) {
			$out[$key] = $value;
			if ($key == 'wt-ftp') {
				$out['wt-usermail'] = __('Email settings');
			}
		}
		if (!isset($out['wt-usermail'])) /* not found wt-ftp tab */ {
			$out['wt-usermail'] = __('Email settings');
		}
		return $out;
	}
...
add_action('wt_iew_plugin_out_settings_form', 'wt_iew_plugin_out_settings_form');
if (!function_exists('wt_iew_plugin_out_settings_form')) {

	function wt_iew_plugin_out_settings_form() {
		include __DIR__.'/user/email-settings.php';
	}

}

define( 'WT_UIEW_VERSION', '1.1.5' );
 ...
 */
function wt_ier_export_csv_linkin_user_listing_page($which) {

	$currentScreen = get_current_screen();

	if ('users' === $currentScreen->id) {
		echo '<a target="_blank" href="' . admin_url('admin.php?page=wt_import_export_for_woo_export&wt_to_export=user') . '" class="button" >' . __('Export to CSV') . ' </a>';
	}
}

add_filter('manage_users_extra_tablenav', 'wt_ier_export_csv_linkin_user_listing_page');