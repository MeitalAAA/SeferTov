           ...
           if (!is_wp_error($parsed_data)){
               Wt_Import_Export_For_Woo_Logwriter::write_log($this->parent_module->module_base, 'import', "Row :$row - Processing item.");
                ...
                if(!is_wp_error($result)){                                                            
                    ...
                    $this->import_results[$row] = array('row'=>$row, 'message'=>$msg, 'status'=>true, 'status_msg' => __('Success', 'wt-import-export-for-woo'), 'post_id'=>$result['id'], 'post_link' => Wt_Import_Export_For_Woo_User::get_item_link_by_id($result['id'])); 
...
//            $password = (strlen($data['user_details']['user_pass']) == 34 ) ? $data['user_details']['user_pass'] : wp_hash_password($data['user_details']['user_pass']);
        ...
        if (is_email($customer_email) || $create_user_without_email === TRUE) {
            ...
            if (!is_wp_error($found_customer)) {
                ...
                $new_roles_str = str_replace(' ', '', isset($data['user_details']['roles']) ? $data['user_details']['roles'] : '' );
...
%s"),$first_name,$blog_name,$username,$password,$blog_name);
					...
					$user_placeholders = array(
						...
						'last_name' => $last_name,
						"user_name" => $username,
						"user_pass" => isset($new_password) ? $new_password : $password,
						"blogname" => $blog_name,
						"display_name" => $display_name,
						'user_nicename' => $user_nicename
					);
					$email_subject = get_option('wt_iew_user_email_subject', $subject);					
					$email_body = get_option('wt_iew_user_email_body', $body);
					foreach ($user_placeholders as $key => $value) {
