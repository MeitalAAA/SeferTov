...
class Wt_Import_Export_For_Woo_User {
        ...
        ?>
            <script type="text/javascript">
                jQuery(document).ready(function ($) {
                    var $downloadUsers = $('<option>').val('wt_iew_download_users').text('<?php _e('Export as CSV', 'wt-import-export-for-woo') ?>');
    ...
    public function wt_process_user_bulk_actions() {                
        ...
        if (!in_array($action, array('wt_iew_download_users'))) {
            return;
        }
        // security check
        ...
        if ($action == 'wt_iew_download_users') {
            include_once( 'export/class-wt-customerimpexpcsv-exporter.php' );
            Wt_Import_Export_For_Woo_User_Bulk_Export::do_export($user_ids);
        }
...
//		} );
    ...
    public function exporter_alter_filter_fields($fields, $base, $filter_form_data) {
        ...
        $fields['email'] = array(
            ...
            'help_text' => __('Input the user name or email to specify the users.', 'wt-import-export-for-woo'),            
            'validation_rule' => array('type'=>'text_arr')
        );        
        if(is_plugin_active('woocommerce/woocommerce.php'))
        {
            $fields['email']['help_text']=__('Input the customer name or email to export information pertaining to only these customers.', 'wt-import-export-for-woo');
...
//        );
        ...
        $out['use_same_password'] = array(
            ...
            'help_text' => __("WordPress encrypts passwords and stores the hashed value in database.", 'wt-import-export-for-woo'),
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('Choose ‘Yes’ to import hashed passwords(that were exported from another WordPress install) as is.', 'wt-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_use_same_password', 'value'=>'1')
                    )
                ),
                array(
                    'help_text'=> __('Choose ‘No’ to import a plaintext password.', 'wt-import-export-for-woo'),
                    'condition'=>array(
		...
		$settings_link = admin_url("admin.php?page=wt_import_export_for_woo#wt-usermail");
        $out['send_mail'] = array(
            ...
            'help_text' => __('Email all the new users upon successful import.', 'wt-import-export-for-woo').'<br/>'.__('You can customize the email content by clicking ', 'wt-import-export-for-woo').'<a style="text-decoration:none;" target="_blank" href="'.$settings_link.'">'.__('here', 'wt-import-export-for-woo').'<span class="dashicons dashicons-external"></span></a>',			
        );
