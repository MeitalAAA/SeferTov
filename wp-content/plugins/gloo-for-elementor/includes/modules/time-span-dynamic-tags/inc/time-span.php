...
namespace Gloo\Modules\Time_Span_Dynamic_Tags;

class Time_Span_Tag extends \Elementor\Core\DynamicTags\Tag {
	...
	public function get_title() {
		return __( 'Time Span Dynamic Tag', 'gloo_for_elementor' );
	}
	...
	public function render() {
		...
		try {
			...
			// 	echo '<pre>'; print_r($start_datetime); echo '</pre>';
			...
			if ( isset( $default_date_options[ $output ] ) ) { // default output
				...
				for ( $i = 0; $i < sizeof( $custom_format_output[0] ); $i ++ ) {
					...
					if ( isset( $get_default_date_placeholders[ $custom_var ] ) ) {
						...
						$output = str_replace('%','', $custom_var);
						$val = $this->get_total_interval( $difference, $output );
						$custom_output = str_replace( $custom_var, $val, $custom_output );
					}
				}
				$value = $custom_output;
			}
