...
var gloo_cookie_form_action_prefix = 'gloo_cookie_form_action';

jQuery(document).ready(function($){

  if (typeof gloo_save_form_cookies != 'function') {
    function gloo_save_form_cookies(current_form) {
      if(current_form.closest('.gloo_cookie_form_action').length >= 1){
        gloo_cookie_form_action_settings = current_form.closest('.gloo_cookie_form_action').attr('data-gloo_cookies_form_action');
        gloo_cookie_form_action_settings = safelyParseJSON(gloo_cookie_form_action_settings);
        //console.log(gloo_cookie_form_action_settings);
        if(typeof gloo_cookie_form_action_settings != 'undefined' && gloo_cookie_form_action_settings.cookie_type != 'undefined'){
          ...
          if(!$.isEmptyObject(cookies_data_object)){
            ...
            if( gloo_cookie_form_action_settings.individual_fields != 'undefined' && gloo_cookie_form_action_settings.individual_fields == 'yes' && gloo_cookie_form_action_settings.form_field_list != 'undefined' && gloo_cookie_form_action_settings.form_field_list.length >= 1){
              for (var key in cookies_data_object_temp) {
                if(!($.inArray( key, gloo_cookie_form_action_settings.form_field_list ) != -1)){
          ...
          if(!$.isEmptyObject(cookies_data_object)){
            for (var key in cookies_data_object) {
              ...
              if(gloo_cookie_form_action_settings.cookie_type == 'cookie'){
                setCookie(key, cookie_string_value);
              }else if(gloo_cookie_form_action_settings.cookie_type == 'local_storage'){
                setLocalStorage(key, cookie_string_value);
              }else if(gloo_cookie_form_action_settings.cookie_type == 'session'){
  ...
  $('body').on('submit', 'form', function(){
    if($('.gloo_cookie_form_action').length >= 1){
